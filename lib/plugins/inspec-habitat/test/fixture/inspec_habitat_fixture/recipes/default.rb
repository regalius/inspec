...
%w{ruby ruby-dev gcc g++ make expect}.each do |required_package|
  package required_package
end

base_dir = node['inspec_habitat_fixture']['base_dir']
...
# InSpec loads `pry` via shell.rb. Pry tries to expand `~`
# The habitat cookbook doesn't define a user so this fails.
# This below is needed until the habitat cookbook adds a
# `User` line to the SystemD unit
